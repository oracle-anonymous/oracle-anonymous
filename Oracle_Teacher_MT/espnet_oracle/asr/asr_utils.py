...
def torch_init(snapshot_path, trainer):
    """Resume from snapshot for pytorch.
    ...
    snap_dict = snapshot_dict["model"]
    del snap_dict['kd_out.weight']
    del snap_dict['kd_out.bias']
    if hasattr(trainer.updater.model, "model"):
        # (for TTS model)
        if hasattr(trainer.updater.model.model, "module"):
            trainer.updater.model.model.module.load_state_dict(snap_dict)
        else:
            trainer.updater.model.model.load_state_dict(snap_dict)
    else:
        # (for ASR model)
        if hasattr(trainer.updater.model, "module"):
            trainer.updater.model.module.load_state_dict(snap_dict)
        else:
            trainer.updater.model.load_state_dict(snap_dict)
