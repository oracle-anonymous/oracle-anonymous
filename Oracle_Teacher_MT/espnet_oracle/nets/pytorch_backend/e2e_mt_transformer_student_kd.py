...
class E2E(MTInterface, torch.nn.Module):
    ...
    def __init__(self, idim, odim, args, ignore_id=-1):
        ...
        self.kd_out = torch.nn.Linear(args.adim, args.adim)
    ...
    def forward(self, xs_pad, ilens, ys_pad, olens, oracle_pad):
        """E2E forward.
        ...
        ys_pad = ys_pad[:, : max(olens)]
        src_mask = (~make_pad_mask(ilens.tolist())).to(xs_pad.device).unsqueeze(-2)
        xs_pad, ys_pad = self.target_forcing(xs_pad, ys_pad)
        # print("ys_pad: "+str(ys_pad.shape[1]))
        hs_pad, hs_mask = self.encoder(xs_pad, src_mask)

        # 2. forward decoder
        ys_in_pad, ys_out_pad = add_sos_eos(ys_pad, self.sos, self.eos, self.ignore_id)
        # print("ys_in_pad: "+str(ys_in_pad.shape[1]))
        ys_mask = target_mask(ys_in_pad, self.ignore_id)
        pred_pad, pred_mask, before_output_layer = self.decoder(ys_in_pad, ys_mask, hs_pad, hs_mask)
        # print("pred_pad: "+str(pred_pad.shape[1]))
        kd_outs = self.kd_out(before_output_layer).view(before_output_layer.size(0), -1, 256)
        # kd_outs = torch.zeros_like(oracle_pad) * kd_outs[0][0][0]

        # 3. compute attention loss
        self.loss = torch.mean(torch.norm((oracle_pad-kd_outs).view(before_output_layer.size(0),-1),2,dim=-1).pow(2)) #self.criterion(pred_pad, ys_out_pad)
