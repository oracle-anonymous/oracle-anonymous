...
from espnet.nets.pytorch_backend.transformer.oracle_decoder import oracle_Decoder
from espnet.nets.pytorch_backend.transformer.oracle_encoder import oracle_Encoder
...
class oracle_Prenet(torch.nn.Module):
    ...
    def __init__(self, idim, n_layers=2, n_units=256, dropout_rate=0.5):
        ...
        """
        super(oracle_Prenet, self).__init__()
        self.dropout_rate = dropout_rate
        self.prenet = torch.nn.ModuleList()
        for layer in six.moves.range(n_layers):
            n_inputs = idim if layer == 0 else n_units
            self.prenet += [
                torch.nn.Sequential(torch.nn.Linear(n_inputs, 40), torch.nn.ReLU(), torch.nn.Linear(40, 20), torch.nn.ReLU(),
                                    torch.nn.Linear(20, n_units), torch.nn.ReLU())
            ]
...
class Decoder(torch.nn.Module):
    ...
    def __init__(
        ...
        decoder_input_layer2 = torch.nn.Sequential(
            oracle_Prenet(
                idim=odim,
                n_layers=prenet_layers,
                n_units=prenet_units,
        ...
        self.oracle_encoder = oracle_Encoder(
            ...
            attention_heads=8,
            linear_units=1024,
            num_blocks=3,  # 2, #elayers,
            input_layer=decoder_input_layer2,
            dropout_rate=0.1,
        ...
        self.oracle_decoder = oracle_Decoder(
            odim=512,  # odim is needed when no prenet is used
            attention_dim=512,
            attention_heads=8,
            linear_units=1024,
            num_blocks=3,  # 2, #dlayers,
            dropout_rate=0.1,
            positional_dropout_rate=0.1,
    ...
    def _reset_parameters(self, init_type, init_enc_alpha=1.0, init_dec_alpha=1.0):
        ...
        self.oracle_encoder.embed[-1].alpha.data = torch.tensor(init_dec_alpha)
        self.oracle_decoder.embed[-1].alpha.data = torch.tensor(init_dec_alpha)
    ...
    def forward(self, hs, hlens, ys, olens):
        ...
        oracle_src_mask = self._source_mask(olens)
        oracle_enc, oracle_enc_masks = self.oracle_encoder(ys, oracle_src_mask)
        ...
        for y in ys.transpose(0, 1):
            ...
            zs_mask = self._oracle_mask(olens)
            zs, _ = self.oracle_decoder(zcs, zs_mask, oracle_enc, oracle_enc_masks)
    ...
    def _oracle_mask(self, olens: torch.Tensor) -> torch.Tensor:
        y_masks = make_non_pad_mask(olens).to(next(self.parameters()).device)
        s_masks = torch.ones(y_masks.size(-1), y_masks.size(-1), device=y_masks.device, dtype=torch.bool)
        return y_masks.unsqueeze(-2) & s_masks