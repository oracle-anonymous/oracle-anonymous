...
from espnet.utils.io_utils import LoadInputsAndTargets_kd
...
teacher_model = "exp/oracle_teacher/results/knowledge_of_the_oracle_teacher/train.en-de.de/"
...
class CustomConverter(object):
    ...
    def __call__(self, batch, device=torch.device("cpu")):
        ...
        oracle_dec_list = []
        for id in utt_id:
            oracle_dec_file = teacher_model + id + '.npy'
            oracle_dec = np.load(oracle_dec_file)[0]
            oracle_dec_list.append(torch.from_numpy(oracle_dec))
        oracle_pad = pad_list(oracle_dec_list, 0.0).to(device)
        ...
        return xs_pad, ilens, ys_pad, olens, oracle_pad
...
def train(args):
    ...
    load_tr = LoadInputsAndTargets_kd(mode="mt", load_output=True)
    # load_cv = LoadInputsAndTargets_kd(mode="mt", load_output=True)
