    ...
    if args.batchsize == 0:
        with torch.no_grad():
            for idx, name in enumerate(js.keys(), 1):
                ...
                feat2 = [js[name]["output"][0]["tokenid"].split()]
                oracle_dec, loss, acc, loss2, acc2 = model.translate(feat, feat2, args, train_args.char_list)
                # oracle_dec, loss, acc = model.translate(feat, feat2, args, train_args.char_list)
                save_numpy = oracle_dec.cpu().detach().numpy()

                save_dir = args.model.split('snapshot')[0] + 'knowledge_of_the_oracle_teacher/' + args.trans_json.split('/')[1] + '/'
                ...
                if not (os.path.isfile(save_name)):
                    np.save(save_name, save_numpy)
                    print("loss: "+str(loss)+" acc: "+str(acc))
                    print("loss2: " + str(loss2) + " acc: " + str(acc2))
                    print("Saving "+ save_name)

                else:
                    print(save_name + " is already exist")
