...
function filter_recordings {
  ...
  # recording-ids.
    ...
    gawk '{print $2}' < $data/segments | sort | uniq > $tmpdir/recordings
    ...
    cp $data/segments{,.tmp}; gawk '{print $2, $1, $3, $4}' <$data/segments.tmp >$data/segments
...
function filter_utts {
  cat $data/utt2spk | gawk '{print $1}' > $tmpdir/utts
  ...
  if [ -f $data/utt2dur ]; then
    cat $data/utt2dur | \
      gawk '{ if (NF == 2 && $2 > 0) { print }}' > $data/utt2dur.ok || exit 1
  ...
  if [ -f $data/utt2num_frames ]; then
    cat $data/utt2num_frames | \
      gawk '{ if (NF == 2 && $2 > 0) { print }}' > $data/utt2num_frames.ok || exit 1
