...
train_config=conf/tuning/train_pytorch_transformer_3_3.yaml
decode_config=conf/decode.yaml

# decoding parameter
trans_model=model.val3.avg.best # set a model to be used for decoding: 'model.acc.best' or 'model.loss.best'

# model average realted (only for transformer)
n_average=3                  # the number of NMT models to be averaged
...
train_dev=tst2013.${src_lang}-${tgt_lang}.${tgt_lang}
...
#if [ -z ${tag} ]; then
#    expname=${train_set}_${backend}_$(basename ${train_config%.*})_${bpemode}${nbpe}_${seed}
#else
#    expname=${train_set}_${backend}_${tag}
#fi
expname=student_training
expdir=exp/${expname}
mkdir -p ${expdir}
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
    ...
    CUDA_VISIBLE_DEVICES=0 mt_student_train.py \
    --config ${train_config} \
    --ngpu ${ngpu} \
    --backend ${backend} \
    ...
    --init_param exp/student_kd/results/snapshot.ep.10
fi



expdir=exp/student_training
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
    ...
    if [[ $(get_yaml.py ${train_config} model-module) = *transformer* ]]; then
        ...
        if ${use_valbest_average}; then
            ...
            opt="--log ${expdir}/results/log --metric acc"
        else
