...
def iterate_data(model, sess, compute_loss, mode, verbose, num_steps=None):
  ...
  ############################################
    if mode == "infer":
      conv_attention_tensors = model.get_conv_attention_tensors(worker_id)
      cur_fetches = [
        model.get_data_layer(worker_id).input_tensors,
        model.get_output_tensors(worker_id),
        conv_attention_tensors,
      ]
    else:
      cur_fetches = [
          model.get_data_layer(worker_id).input_tensors,
  ...
  while True:
    ...
    for worker_id, fetches_val in fetches_vals.items():
      if compute_loss:
        ...
        ########################################
        if mode == "infer":
          inputs, outputs, conv_attentions = fetches_val[:3]
        else:
      ...
      if size_defined:
        ...
        if total_samples[worker_id] > dl_sizes[worker_id]:
          ...
          ########################################
          if mode == "infer":
            conv_attentions = model.clip_last_batch(conv_attentions, last_batch_size)
          ########################################
      ...
      if mode == 'eval':
        ...
        ########################################
        results_per_batch.append(model.infer(inputs, outputs, conv_attentions))
        ########################################
      else:
...
def get_base_config(args):
  ...
  parser.add_argument("--config_file", default='example_configs/speech2text/jasper_mini_3gpu.py',
  ...
  parser.add_argument('--enable_logs', dest='enable_logs', action='store_true', default=True,
