...
class Speech2TextDataLayer(DataLayer):
  ...
  def __init__(self, params, model, num_workers, worker_id):
    ...
    * **sample_freq** (int) --- required for precompute_mel_basis
    ...
    if self.params['mode'] == 'train':
      cols = ['wav_filename', 'transcript', 'teacher_kd']
  ...
  def build_graph(self):
    with tf.device('/cpu:0'):
      ...
      if self.params['mode'] == 'train':
        ...
        self._dataset = self._dataset.map(
            lambda line: tf.py_func(
                self._parse_audio_transcript_attention_element,
                [line],
                [self.params['dtype'], tf.int32, tf.int32, tf.int32, tf.float32, tf.float16],
                stateful=False,
            ),
        ...
        if self.params['max_duration'] > 0:
          self._dataset = self._dataset.filter(
              lambda x, x_len, y, y_len, duration, attention_map2:
        ...
        self._dataset = self._dataset.map(
            lambda x, x_len, y, y_len, duration, attention_map2:
            [x, x_len, y, y_len, attention_map2],
            num_parallel_calls=8,
        )
        self._dataset = self._dataset.padded_batch(
            ...
            padded_shapes=([None, self.params['num_audio_features']],
                           1, [None], 1, [None]),
            padding_values=(
                tf.cast(0, self.params['dtype']), 0, self.target_pad_value, 0, tf.cast(0, tf.float16)),
        )
      ...
      if self.params['mode'] == 'train':
        x, x_length, y, y_length, attention_map2 = self._iterator.get_next()
    #    attention_map.set_shape([self.params['batch_size'], None])
        attention_map2.set_shape([self.params['batch_size'], None])
      ...
      if self.params['mode'] == 'train':
        ...
        #self._input_tensors['conv_attention_tensors'] = [attention_map]
        self._input_tensors['teacher_at'] = [attention_map2]
...
#####################################################################
  def _parse_audio_transcript_attention_element(self, element):
    audio_filename, transcript, attention_map2_filename = element
    if not six.PY2:
      transcript = str(transcript, 'utf-8')
      audio_filename = str(audio_filename, 'utf-8')
    ...
    if os.path.exists(attention_map2_filename):#and os.path.exists(attention_map2_filename):
#        attention_map = get_attention_maps_from_file(attention_map_filename)
        ...
        print("Thers's no: ",attention_map2_filename)
    return source.astype(self.params['dtype'].as_numpy_dtype()), \
        np.int32([len(source)]), \
        np.int32(target), \
        np.int32([len(target)]), \
