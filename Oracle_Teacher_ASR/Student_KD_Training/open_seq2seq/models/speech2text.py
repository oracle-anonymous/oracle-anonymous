...
class Speech2Text(EncoderDecoderModel):
  ...
  def _build_forward_pass_graph(self, input_tensors, gpu_id=0):
    ...
    with tf.variable_scope("ForwardPass"):
      ...
      if self.mode == "infer":
        ...
        save_conv_attention = conv_len
      ######################################################################################
      if self.mode == "train":

        vocab_size = decoder_output['logits'].shape[2]
        batch_size = decoder_output['logits'].shape[1]

        teacher_conv = tf.reshape(input_tensors['teacher_at'][0],[batch_size,900,512])
        ...
        student_connect2 = encoder_output['student_connect2']
        teacher_conv,_ = tf.split(teacher_conv,[conv_len,-1],axis=1)
        hint_loss = tf.reduce_mean(tf.square(tf.norm(tf.reshape(tf.cast(teacher_conv,tf.float32)-tf.cast(student_connect2,tf.float32),[batch_size,-1]),ord=2,axis=1)))
      ...
      ######################################################################################
        ...
        if self.dump_outputs:
          ...
          return loss, [model_logits], save_conv_attention

    if self.mode == "train":
      return (loss/10000000000)+hint_loss, model_outputs, hint_loss 
    elif self.mode == "eval":
      return loss, model_outputs
    else:
      return loss, model_outputs, save_conv_attention
  ...
  def finalize_evaluation(self, results_per_batch, training_step=None):
    ...
    #######################
    wer_file = os.path.join(self.params['logdir'], 'wer.csv')

    with open(wer_file,'a') as f:
     f.write(str(total_wer))
     f.write('\n')
    ########################
  ...
  def infer(self, input_values, output_values, conv_attention_values):
    ...
    deco_print("Saving attention maps")

    k=0

    for i in ids:
      filename = wav_filename_list[i]
      savename = filename.replace('/data/librispeech/LibriSpeech/', '/save_path/').replace('.wav', '.save')
      attention_value = conv_attention_values[k]

      if k >= self.params['batch_size_per_gpu']:
        deco_print("Something wrong!")

      dir = os.path.dirname(savename)  # savename
      if not (os.path.isdir(dir)):
        os.makedirs(dir)

      if not (os.path.isfile(savename)):  # savename
