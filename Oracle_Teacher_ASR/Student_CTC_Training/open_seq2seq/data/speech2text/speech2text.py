...
from .speech_utils import get_speech_features_from_file, get_speech_features, load_features
...
class Speech2TextDataLayer(DataLayer):
  ...
  def create_feed_dict_2(self, model_in):
    """ Creates the feed dict for interactive infer
    ...
    # audio_length_arr = []
    #
    # audio_arr.append(model_in['audio_features'])
    # audio_length_arr.append(model_in[1])
    #
    # max_len = np.max(audio_length_arr)
    # pad_to = self.params.get("pad_to", 8)
    # if pad_to > 0 and self.params.get('backend') == 'librosa':
    ...
    feed_dict = {
        self._x: model_in['audio_features'],
        self._x_length: model_in['features_length'],
        self._x_id: model_in['id'],
    }
    return feed_dict
  ...
  def _parse_audio_transcript_element(self, element):
    ...
    #source, audio_duration = load_features(audio_filename,data_format='hdf5')
