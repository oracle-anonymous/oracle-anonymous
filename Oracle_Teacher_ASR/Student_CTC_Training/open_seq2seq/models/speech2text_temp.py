...
#        len(true_text.split())
    ...
    sample_wer = levenshtein(true_text.replace(' ',''), pred_text.replace(' ','')) / len(true_text.replace(' ',''))
...
#      total_word_count += len(true_text.split())
      ...
      total_word_lev += levenshtein(true_text.replace(' ',''), pred_text.replace(' ',''))
      total_word_count += len(true_text.replace(' ',''))
  ...
  def infer(self, input_values, output_values):
    ...
    for i in ids:
      ...
      if wav_filename_list[i] != wav_filename_list[ids][k]:
        print(wav_filename_list[i])
        print(wav_filename_list[ids][k])
        raise ValueError('Something wrong with wav filename')
      filename = wav_filename_list[i]
      with open('201124_jasper_teacher_noisy_infer.txt', 'a') as f:
        f.write(filename+ ',' + preds[k] +',check'+ '\n')
      k+=1 
