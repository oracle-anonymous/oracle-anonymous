...
from .speech_utils import get_speech_features_from_file, get_speech_features, get_attention_maps_from_file
...
class Speech2TextDataLayer(DataLayer):
  ...
  def __init__(self, params, model, num_workers, worker_id):
    ...
    * **sample_freq** (int) --- required for precompute_mel_basis
    ...
    ###
    self._batch_num = self._size // self.params['batch_size']
    ###
    self._dataset = None
    self._iterator = None
  ...
  def build_graph(self):
    with tf.device('/cpu:0'):
      ...
      ######################################################
      if self.params['mode'] == 'train': # or self.params['mode'] == 'infer':
      ...
      ###########################################################
        ...
        self._dataset = tf.data.Dataset.from_tensor_slices(
            np.hstack((indices[:, np.newaxis], self._files[:,0][:, np.newaxis], self._files[:,1][:, np.newaxis]))
        )
        if self.params['shuffle']:
            self._dataset = self._dataset.shuffle(self._size)
        self._dataset = self._dataset.repeat()
        ...
        self._dataset = self._dataset.map(
            lambda line: tf.py_func(
                self._parse_audio_transcript_oracle_infer,
                [line],
                [self.params['dtype'], tf.int32, tf.int32, tf.int32, tf.int32, tf.float32],
                stateful=False,
            ),
            num_parallel_calls=8,
        )
        if self.params['max_duration'] > 0:
            self._dataset = self._dataset.filter(
                lambda x, x_len, idx, y, y_len, duration:
                tf.less_equal(duration, self.params['max_duration'])
            )
        if self.params['min_duration'] > 0:
            self._dataset = self._dataset.filter(
        ...
        self._dataset = self._dataset.padded_batch(
            ...
            padded_shapes=([None, self.params['num_audio_features']],
                           1, 1, [None], 1),
            padding_values=(
                tf.cast(0, self.params['dtype']), 0, 0, self.target_pad_value, 0),
        )
      ...
      if self.params['mode'] == 'train': # or self.params['mode'] == 'infer':
        ...
        x, x_length, x_id, y, y_length = self._iterator.get_next()
      ...
      if self.params['mode'] == 'train':
        ...
        self._input_tensors['pad'] = num_pad
...
#####################################################################
  def _parse_audio_transcript_oracle_infer(self, element):
      idx, audio_filename, transcript = element
      if not six.PY2:
          transcript = str(transcript, 'utf-8')
          audio_filename = str(audio_filename, 'utf-8')
      if self.params['bpe']:
...
#####################################################################
  def _parse_audio_transcript_attention_element(self, element):
    audio_filename, transcript, attention_map_filename, attention_map2_filename, prediction = element
    if not six.PY2:
      transcript = str(transcript, 'utf-8')
      audio_filename = str(audio_filename, 'utf-8')
      attention_map_filename = str(attention_map_filename, 'utf-8')
      attention_map2_filename = str(attention_map2_filename, 'utf-8')
      prediction = str(prediction, 'utf-8')
    if self.params['bpe']:
      target_indices = self.sp.EncodeAsIds(transcript)
      pred_indices = self.sp.EncodeAsIds(prediction)
    else:
      target_indices = [self.params['char2idx'][c] for c in transcript]
      pred_indices = [self.params['char2idx'][c] for c in prediction]
    if self.autoregressive:
      target_indices = target_indices + [self.end_index]
      pred_indices = pred_indices + [self.end_index]
    target = np.array(target_indices)
    pred = np.array(pred_indices)
    ...
    if os.path.exists(attention_map_filename) and os.path.exists(attention_map2_filename):
        attention_map = get_attention_maps_from_file(attention_map_filename)
        attention_map2 = get_attention_maps_from_file(attention_map2_filename)
    ...
    return source.astype(self.params['dtype'].as_numpy_dtype()), \
        ...
        np.int32([len(target)]), \
        np.float32([audio_duration]), \
        np.float16(attention_map), \
        np.float16(attention_map2), \
        np.int32(pred), \
        np.int32([len(pred)])
...
#   def _parse_audio_element_with_text(self, id_and_audio_filename):
#     idx, audio_filename = id_and_audio_filename
#         # audio_filename, transcript, attention_map_filename, attention_map2_filename, prediction = element
# # 495     if not six.PY2:
# # 496       transcript = str(transcript, 'utf-8')
# # 497       audio_filename = str(audio_filename, 'utf-8')
# # 498       attention_map_filename = str(attention_map_filename, 'utf-8')
# # 499       attention_map2_filename = str(attention_map2_filename, 'utf-8')
# # 500       prediction = str(prediction, 'utf-8')
# # 501     if self.params['bpe']:
# # 502       target_indices = self.sp.EncodeAsIds(transcript)
# # 503       pred_indices = self.sp.EncodeAsIds(prediction)
# # 504     else:
# # 505       target_indices = [self.params['char2idx'][c] for c in transcript]
# # 506       pred_indices = [self.params['char2idx'][c] for c in prediction]
# # 507     if self.autoregressive:
# # 508       target_indices = target_indices + [self.end_index]
# # 509       pred_indices = pred_indices + [self.end_index]
# # 510     target = np.array(target_indices)
# # 511     pred = np.array(pred_indices)
#
#     source, audio_duration = get_speech_features_from_file(
#         audio_filename,
#         params=self.params
