...
def train(train_model, eval_model=None, debug_port=None, custom_hooks=None):
  ...
  if hvd is not None:
    # pylint: disable=no-member
    sess_config.gpu_options.visible_device_list = '2' #str(hvd.local_rank())
...
#
  ...
  if restoring:
    ...
    #   if 'global_step' not in var.name.split(':')[0]:
    #     if 'fully_connected_ctc_decoder' in var.name.split('/')[1] or 'w2l' in var.name.split('/')[1]:
  ...
  while True:
    ...
    try:
      ...
      if step % iter_size == 0:
        ...
        #########################################
        if step % 500 == 0:
          at_loss = sess.run([train_model.at_loss], feed_dict)
          print("Attention transfer loss: ", at_loss)
        #########################################
