...
class Model:
  ...
  def __init__(self, params, mode="train", hvd=None):
    ...
    * **larc_params** --- dictionary with parameters for LARC (or LARS)
    ...
    ######################################
    if self.on_horovod:
      self._conv_attention = None
    else:
      self._conv_attentions = [None] * self.num_gpus
    ######################################
    ##############
    self.at_loss = None
    self.lr = None
  ...
  def compile(self, force_var_reuse=False, checkpoint=None):
    ...
    if not self.on_horovod:  # not using Horovod
      ...
      for gpu_cnt, gpu_id in enumerate(self._gpu_ids):
        with tf.device("/gpu:{}".format(gpu_id)), tf.variable_scope(
          ...
          if self.params.get("use_trt", False):
            ...
            if self._mode == "train":
                loss, self._outputs[gpu_cnt], at_loss = self._build_forward_pass_graph(
                    input_tensors,
                    gpu_id=gpu_cnt
                )
                ...
                loss, self._outputs[gpu_cnt], self._conv_attentions[gpu_cnt] = self._build_forward_pass_graph(
                    input_tensors,
                    gpu_id=gpu_cnt
                )
      ...
      if self._mode == "train":
        ...
        self.at_loss = tf.reduce_mean(at_losses)
      if self._mode == "eval":
      ...
      with tf.device("/gpu:0"), tf.variable_scope(
        ...
        if self.params.get("use_trt", False):
          ...
          # Build regular TF graph
          if self._mode == "train":
              all_loss, self._output, at_loss = self._build_forward_pass_graph(
                  input_tensors,
                  gpu_id=0
              )
          elif self._mode == "eval":
              ...
              all_loss, self._output, self._conv_attention = self._build_forward_pass_graph(
                  input_tensors,
                  gpu_id=0
              )
    ...
    if self._mode == "train":
      ...
      self.train_op, self.lr = optimize_loss(
...
####################################################
  ...
  ####################################################
  def get_conv_attention_tensors(self, worker_id=0):
      if self.on_horovod:
          return self._conv_attention
      else:
          return self._conv_attentions[worker_id]
  ####################################################
