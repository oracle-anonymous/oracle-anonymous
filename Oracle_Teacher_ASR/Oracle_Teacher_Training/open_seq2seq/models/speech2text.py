...
from open_seq2seq.parts.cnns.conv_blocks import conv_bn_actv,conv_actv
...
class Speech2Text(EncoderDecoderModel):
  ...
  def _build_forward_pass_graph(self, input_tensors, gpu_id=0):
    ...
    if self.mode == "train" or self.mode == "eval" or self.mode == "infer":
    ...
    with tf.variable_scope("ForwardPass"):
      ...
      if self.mode == "train" or self.mode =="eval" or self.mode =="infer":
        ...
        vocab_size =  self.get_data_layer().params['tgt_vocab_size']
        num_heads = 4
 
        max_src_len = tf.reduce_max(source_tensors[1])
        pad = input_tensors['pad']
        conv_len = (max_src_len + pad) // 2
        teacher_conv = encoder_output['outputs']
        teacher_conv = tf.split(teacher_conv,[conv_len,-1],axis=1)[0]
        
        max_tgt_len = tf.reduce_max(target_tensors[1])

        src_mask = tf.logical_not(tf.sequence_mask(lengths=source_tensors[1], maxlen=conv_len))
        tgt_mask = tf.logical_not(tf.sequence_mask(lengths=target_tensors[1], maxlen=max_tgt_len))

        training = encoder_output['training']
        dropout_rate = 0.1


        with tf.variable_scope("oracle_encoder", reuse=tf.AUTO_REUSE):
           tgt_dim = 512
           tgt_embeddings = get_token_embeddings(vocab_size-1, tgt_dim, zero_pad=True)
           target_embedding = tf.cast(tf.nn.embedding_lookup(tgt_embeddings, target_tensors[0]),tf.float16)

           target_pe = tf.split(positional_encoding(target_embedding, 600),[max_tgt_len, -1], axis=1)[0]
           target_embedding += tf.cast(target_pe,tf.float16)
           enc = tf.layers.dropout(target_embedding,dropout_rate,training=training)
           ...
           for i in range(2):
               with tf.variable_scope("num_blocks_{}".format(i), reuse=tf.AUTO_REUSE):
                   enc = multihead_attention(queries=enc,
                                             ...
                                             causality=False)
                   # feed forward
                   enc = ff(enc, training=training, num_units=[2048, tgt_dim])


        with tf.variable_scope("oracle_decoder", reuse=tf.AUTO_REUSE):
            teacher_pe = tf.split(positional_encoding(teacher_conv, 900),[conv_len, -1], axis=1)[0]
            teacher_conv += tf.cast(teacher_pe,tf.float16)
            teacher_conv = tf.layers.dropout(teacher_conv,dropout_rate,training=training)

            for i in range(2):
                with tf.variable_scope("num_blocks_{}".format(i), reuse=tf.AUTO_REUSE):
                    teacher_conv = multihead_attention(queries=teacher_conv,
                                              keys=teacher_conv,
                                              values=teacher_conv,
                                              key_masks=src_mask,
                                              num_heads=num_heads,
                    ...
                    ### Feed Forward
                    teacher_conv = ff(teacher_conv, training=training, num_units=[2048, tgt_dim])
                    

        with tf.variable_scope("ctc_second_dec", reuse=tf.AUTO_REUSE):
          encoder_output['outputs'] = teacher_conv
        ...
        if self.mode != "infer":
            with tf.variable_scope("ctc_Loss"):
              ...
              loss, target_sparse = self.loss_computator.compute_loss(loss_input_dict)
    ...
    if self.mode == "train":
      ...
      _, padding = tf.split(teacher_conv, [-1, 1], 1)
      padding = tf.tile(padding,[1,900,1])
      concat_with_pad = tf.concat([teacher_conv,padding],1)
      teacher_conv, _ = tf.split(concat_with_pad,[900,-1],1)
      deco_print("Inference Mode. Loss part of graph isn't built.")
      loss = None
      if self.dump_outputs:
        model_logits = decoder_output.get("logits", None)
        return loss, [model_logits], teacher_conv
      else:
        return loss, model_outputs, teacher_conv
  ...
  def infer(self, input_values, output_values, conv_attention_values):

    wav_filename_list = self.get_data_layer().all_files
    ids = input_values['source_ids']
    ids = np.hstack(ids)

    deco_print("Saving the knowledge")

    k=0

    for i in ids:
      filename = wav_filename_list[i]
      filename = filename[0]
      savename = filename.replace('/data/librispeech/LibriSpeech/', '/knowledge_of_the_oracle_teacher/').replace('.wav', '.new')
      attention_value = conv_attention_values[k]

      if k >= self.params['batch_size_per_gpu']:
        deco_print("Something wrong!")
      ...
      if not (os.path.isfile(savename)):
        with open('extracting_list.txt', 'a') as f:
          f.write(filename + '\n')
        with open(savename, 'w') as f:
          attention_value.tofile(f)

        k += 1
        print(k)

      else:
        deco_print("Already exist")
        k += 1
        return deco_print("Finished saving")
